"""Binary Ninja plugin entry point for IRIS."""
...
if bn is not None:
    ...
    from .iris.core.host import set_binary_ninja_context
    from .iris.binja.ui.actions import ACTION_DEFS, build_context
    from .iris.binja.ui.panel import IRISPanel
    from .iris.binja.tools.common import get_function_at, get_function_name
    ...
    IRISPanel = Any  # type: ignore[misc,assignment]
    ACTION_DEFS = ()
    bnui = None
...
IRIS_SIDEBAR_NAME = "IRIS"

_PANEL: Optional[IRISPanel] = None
_LAST_BV: Any = None
_REGISTERED = False
...
def _get_sidebar_panel(create_if_missing: bool = True) -> Optional[IRISPanel]:
    sidebar = _active_sidebar()
    if sidebar is None:
        return None
    try:
        widget = sidebar.widget(IRIS_SIDEBAR_NAME)
        if widget is None and create_if_missing:
            sidebar.activate(IRIS_SIDEBAR_NAME)
            widget = sidebar.widget(IRIS_SIDEBAR_NAME)
        if widget is not None and hasattr(widget, "panel"):
            panel = getattr(widget, "panel")
            if isinstance(panel, IRISPanel):
                return panel
    except Exception:
        return None
    return None
...
def _ensure_panel(bv: Any, address: Optional[int] = None) -> IRISPanel:
    global _PANEL
...
def _register_sidebar() -> None:
    ...
    class IRISSidebarWidget(SidebarWidget):
        def __init__(self, view_frame, binary_view):
            super().__init__(IRIS_SIDEBAR_NAME)
            self.view_frame = view_frame
            self.binary_view = binary_view
            self.panel = IRISPanel()
    ...
    class IRISSidebarWidgetType(SidebarWidgetType):
        def __init__(self):
            icon_dir = os.path.join(os.path.dirname(__file__), "assets")
            icon = QImage(os.path.join(icon_dir, "iris_icon.png"))
            if icon.isNull():
                icon = QImage(":/icons/sidekick-assistant.png")
            if icon.isNull():
                icon = QImage(os.path.join(icon_dir, "chat.png"))
            SidebarWidgetType.__init__(self, icon, IRIS_SIDEBAR_NAME)

        def createWidget(self, frame, data):
            if data is None:
                return None
            return IRISSidebarWidget(frame, data)
    ...
    try:
        Sidebar.addSidebarWidgetType(IRISSidebarWidgetType())
